...
class Player
{
	...
	bool stop;//done by noha
	bool chosenInCard4;//done by noha
	bool freeze;
	int countFreeze;
	int freezeTurns;
	int BenefitAll;
...
public:
	...
	void SetStop(bool s);//done by noha
	bool GetStop();//done by noha
	void IsChosen();//done by noha
	int GetPlayerNo();//noha
	void setFreeze(bool x, int n);
	//bool getFreeze();
	void BenefitALL(int y);
};
