...
{
	int hpos = cellPos.HCell();  // Get horizontal position
	int x = hpos * (110);  // horizontal position * cell width to get x coordinate of top left corner
	return x; // return x-coordinate of top left corner
}
...
{
	int vpos = cellPos.VCell(); // Get vertical position
	int y = vpos * 55+55; // 
	return y; // return y-coordinate of top left corner
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_Add_Coin_Set] = "images\\Coin_set.jpg";
	MenuItemImages[ITM_ADD_CARD] = "images\\Menu_Card.jpg";	
	MenuItemImages[ITM_Copy_Card_Or_Coins] = "images\\Copy.jpg";
	MenuItemImages[ITM_Cut_Card_Or_Coins] = "images\\Cut.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_OfferMortgage] = "images\\Offer.jpg";
	MenuItemImages[ITM_AcceptMortage] = "images\\Accept.jpg";
	MenuItemImages[ITM_ReturnMortage] = "images\\Return.jpg";
	MenuItemImages[ITM_SellCity] = "images\\Sell.jpg";
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX,cellStartY,cellStartX+110,cellStartY+55,FILLED,0,0);
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.GetCellNum() > toCell.GetCellNum() || fromCell.HCell() > toCell.HCell())
		return;
	...
	for (int i = GetCellStartY(toCell) + UI.CellHeight; i <= GetCellStartY(fromCell); i += UI.CellHeight) {
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int xSH[4] = { x12,x12 - xChange,x12,x12 + xChange }; //Snake Head
	int ySH[4] = { y1,y1 + yChange,y1 + yChange * 2,y1 + yChange }; //Snake Head
...
{
	image picture("images\\Coin_Set.jpg");
	int cellStartX = GetCellStartX(cellPos);
	int cellStartY = GetCellStartY(cellPos);
	int w = 0, h = 0;
	int x = UI.CellWidth ;
	int y = UI.CellHeight;
	//Draw the coin
	pWind->DrawImage(picture, cellStartX, cellStartY, x, y);
