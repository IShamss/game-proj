...
public:
	...
	void Removeobjectfromcellnodelete(const CellPosition& pos);
	...
	void SetCoinClipboard(CoinSet* coin);
	CoinSet* GetCoinClipboard() const;

	void SetEndGame(bool endGame);	 // A setter for endGame data member
	bool GetEndGame() const;		 // A getter for endGame data member

	bool Getfromcopy();
	void Setfromcopy(bool x);

	void AdvanceCurrentPlayer();
	// Increments the currPlayerNum and if reaches MaxPlayerCount reset to 0 (using %)

	void SetcurrPlayerNumber(int curr);
	int GetcurrPlayerNumber();

	Cell* GetCell(CellPosition& cpos) const;
	...
	void Resetcity(); // Resets ownership of city cards for new game
	void ApplyforsameCairo(Cairo* x);
	void ApplyforsameAswan(Aswan* x);
	void ApplyforsameLuxor(Luxor* x);
	void ApplyforsameAlex(Alexandria* x);
	void ApplyforsameHurg(Hurghada* x);
	...
	int cardCount();
	int coinCount();
	void Save(ofstream& output, int obj);
	void RemoveAllObjects();

	void MovePlayersBackWard(Player* pCurrPlayer);//done by noha
	Player*& GetClickedPlayer(CellPosition pos);//done by noha
	void firing(Player* curP);//done by noha
	void FreezePlayers(Player* currentPlayer, bool y, int n);
	void LoadGameObjects(int, char, ifstream&, Grid*);
};
